...
export default function Chat() {
  ...
  const handleReaction = (reaction) => {
    ...
    if (selectedMessage) {
      const { id, sender, receiver, time, reactions: existingReactions } = selectedMessage;
      const updatedReactions = [...existingReactions, reaction];
      
      database.ref(`msgS/${id}`).update({
   ...
   const renderReactionsModal = () => {
    return (
      <Modal visible={selectedMessage !== null} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <Text style={styles.modalTitle}>Select a reaction:</Text>
          <View style={styles.reactionsContainer}>
            {/* Render your available reactions here */}
            {/* For example, you can use emojis or custom images */}
          ...
          <TouchableOpacity onPress={() => setSelectedMessage(null)} style={styles.closeModalButton}>
  ...
  const handleOpenReactionsModal = (message) => {
...
const styles = StyleSheet.create({
  ...
  currentUserMessage: {
    alignSelf: "flex-end",
    backgroundColor: "rgb(73 131 255)",
    color: "#fff"
  }, currentUserMessage2: {
    alignSelf: "center",
