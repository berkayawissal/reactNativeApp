...
export default function Chat() {
  const defaultAvatar = 'URL_DE_VOTRE_IMAGE_PAR_DEFAUT';
    ...
    return (
      <View style={styles.messageContainer}>
        ...
        <View style={styles.reactionsContainer}>
          {reactionCount > 0 && (
            <Text style={styles.reactionCountText}>{`${reactionCount} ${reactionCount > 1 ? 'reactions' : 'reaction'}`}</Text>
          )}
          {Object.keys(reactions[item.id] || {}).map((emoji, index) => (
  ...
  const renderReactions = () => {
    ...
    const handleReaction = (messageId, reaction) => {
      ...
      const messageRef = database.ref(`msgS/${messageId}/reactions`);
